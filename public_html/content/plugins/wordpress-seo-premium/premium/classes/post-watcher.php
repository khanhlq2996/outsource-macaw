...
class WPSEO_Post_Watcher extends WPSEO_Watcher implements WPSEO_WordPress_Integration {
	...
	public function detect_post_trash( $post_id ) {
		...
		if ( ! empty( $url ) ) {
			...
			// Format the message.
			$message = sprintf(
				/* translators: %1$s: SEO Tool Premium, %2$s: List with actions, %3$s: <a href=''>, %4$s: </a>, %5$s: Slug to post */
	...
	public function detect_post_untrash( $post_id ) {
		...
		if ( $redirect ) {
			...
			// Format the message.
			$message = sprintf(
				/* translators: %1$s: SEO Tool Premium, %2$s: <a href='{undo_redirect_url}'>, %3$s: </a>, %4$s: Slug to post */
	...
	protected function get_undo_slug_notification() {
		/* translators: %1$s: SEO Tool Premium, %2$s and %3$s expand to a link to the admin page. */
		return __(
	...
	protected function get_delete_notification() {
		/* translators: %1$s: SEO Tool Premium, %2$s: List with actions, %3$s: <a href='{post_with_explaination.}'>, %4$s: </a> */
		return __(
