...
class WPSEO_Redirect_Handler {
	...
	protected function add_redirect_by_header() {
		 ...
		 * (only available in SEO Tool Premium, defaults to true, which is adding it)
		 *
		 * @api bool
		 */
		if ( apply_filters( 'wpseo_add_x_redirect', true ) === true ) {
			header( 'X-Redirect-By: SEO Tool Premium' );
		}
	}
	...
	protected function redirect( $location, $status = 302 ) {
		if ( ! function_exists( 'wp_redirect' ) ) {
			require_once ABSPATH . 'libs/pluggable.php';
		}

		wp_redirect( $location, $status, 'SEO Tool Premium' );
		exit;
	}
