...
class WPSEO_Help_Center {
	...
	protected function format_data( array $tabs ) {
		...
		if ( $is_premium === false ) {
			$formatted_data['videoDescriptions'][] = array(
				...
				'linkText'    => sprintf( __( 'Get %s', 'wordpress-seo' ), 'SEO Tool Premium' ),
			);
	...
	private function add_contact_support_item() {
		/* translators: %s: expands to 'SEO Tool Premium'. */
		$popup_title   = sprintf( __( 'Email support is a %s feature', 'wordpress-seo' ), 'SEO Tool Premium' );
		...
		/* translators: %s: expands to 'SEO Tool Premium'. */
		$popup_content .= '<p>' . sprintf( __( 'Other benefits of %s for you:', 'wordpress-seo' ), 'SEO Tool Premium' ) . '</p>';
