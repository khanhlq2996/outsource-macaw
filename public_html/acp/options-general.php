...
require_once(ABSPATH . 'acp/includes/translation-install.php');
		...
		<td>
			...
			if ( defined( 'WPLANG' ) && ( '' !== WPLANG ) && $locale !== WPLANG ) {
				if ( is_multisite() && current_user_can( 'manage_network_options' )
					...
					?>
					<p class="description">
						<strong><?php _e( 'Note:' ); ?></strong> <?php printf( __( 'The %1$s constant in your %2$s file is no longer needed.' ), '<code>WPLANG</code>', '<code>config.php</code>' ); ?>
					</p>
					<?php
				}
				_deprecated_argument( 'define()', '4.0.0', sprintf( __( 'The %1$s constant in your %2$s file is no longer needed.' ), 'WPLANG', 'config.php' ) );
			}
			?>
		</td>
	</tr>
...
<?php include(ABSPATH . 'acp/admin-footer.php'); ?>
