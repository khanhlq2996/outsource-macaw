 ...
 * @global string $self
 */
$self = preg_replace( '|^.*/acp/network/|i', '', $_SERVER['PHP_SELF'] );
$self = preg_replace( '|^.*/acp/|i', '', $self );
...
function _wp_menu_output( $menu, $submenu, $submenu_as_parent = true ) {
	...
	foreach ( $menu as $key => $item ) {
		...
		if ( $is_separator ) {
			...
			if ( ! empty( $menu_hook ) || ( ( 'index.php' != $submenu_items[0][2] ) && file_exists( WP_PLUGIN_DIR . "/$menu_file" ) && ! file_exists( ABSPATH . "/acp/$menu_file" ) ) ) {
			...
			if ( ! empty( $menu_hook ) || ( ( 'index.php' != $item[2] ) && file_exists( WP_PLUGIN_DIR . "/$menu_file" ) && ! file_exists( ABSPATH . "/acp/$menu_file" ) ) ) {
				$admin_is_parent = true;
		...
		if ( ! empty( $submenu_items ) ) {
			...
			foreach ( $submenu_items as $sub_key => $sub_item ) {
				...
				if ( ! empty( $menu_hook ) || ( ( 'index.php' != $sub_item[2] ) && file_exists( WP_PLUGIN_DIR . "/$sub_file" ) && ! file_exists( ABSPATH . "/acp/$sub_file" ) ) ) {
