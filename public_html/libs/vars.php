...
if ( is_admin() ) {
	// acp pages are checked more carefully
	if ( is_network_admin() ) {
		preg_match( '#/acp/network/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	} elseif ( is_user_admin() ) {
		preg_match( '#/acp/user/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	} else {
		preg_match( '#/acp/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	}
	...
	if ( '' === $pagenow || 'index' === $pagenow || 'index.php' === $pagenow ) {
		...
		if ( '.php' !== substr( $pagenow, -4, 4 ) ) {
			$pagenow .= '.php'; // for Options +Multiviews: /acp/themes/index.php (themes.php is queried)
		}
	}
} else {
