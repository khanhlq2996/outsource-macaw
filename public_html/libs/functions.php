...
function do_robots() {
	...
	if ( '0' == $public ) {
		...
		$output  .= "Disallow: $path/acp/\n";
		$output  .= "Allow: $path/acp/admin-ajax.php\n";
	}
...
function wp_upload_bits( $name, $deprecated, $bits, $time = null ) {
	...
	/** This filter is documented in acp/includes/file.php */
 ...
 *
 * Will remove acp links to retrieve only return URLs not in the acp
 * directory.
 *
 * @since 2.6.0
 *
...
function wp_guess_url() {
	if ( defined( 'WP_SITEURL' ) && '' != WP_SITEURL ) {
		...
		if ( strpos( $_SERVER['REQUEST_URI'], 'acp' ) !== false || strpos( $_SERVER['REQUEST_URI'], 'login.php' ) !== false ) {
			$path = preg_replace( '#/(acp/.*|login.php)#i', '', $_SERVER['REQUEST_URI'] );
